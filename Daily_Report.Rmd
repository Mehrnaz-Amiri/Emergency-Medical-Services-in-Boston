...
MyDate <- "2020-11-27"
cad.daily <- CAD[CAD$run_dt == MyDate,]
...
pcr_daily <- cbind(a1,a2,a3,a4,a6,a9,a10,a11,a12,a13,a14)


m <- as.POSIXct(pcr_daily$eTimes.03,format="%Y-%m-%d %H:%M:%S",tz=Sys.timezone())
h <- ymd_hms(pcr_daily$eTimes.03,tz=Sys.timezone())
pcr_daily$eTimes.03 <- as.Date(h, format = "%Y-%m-%d %H:%M:%S'")
pcr_daily <- pcr_daily[pcr_daily$eTimes.03 == MyDate,]

cad.daily$num_1 <- gsub("E*", "", cad.daily$num_1)
cad.pcr <- merge(cad.daily, pcr_daily, by.x = "num_1", by.y = "eResponse.03")
```
...
# Results For `r MyDate`
\

\

## Number of Incidents by Shift on `r MyDate`
...
## Call History by Disposition on `r MyDate`
\

```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=""}

cad.pcr$eDisposition.12 <- ifelse(cad.pcr$eDisposition.12 == "" | cad.pcr$eDisposition.12 == "Canceled (Prior to Arrival At Scene)" | cad.pcr$eDisposition.12 == "No Patient Found (Canceled on Scene)" | cad.pcr$eDisposition.12 == "Treated, Transferred to Other EMS", NA, ifelse(cad.pcr$eDisposition.12 == "Dead at Scene - No Resus - Not Transported", "Referred to the Medical Examiner", as.character(cad.pcr$eDisposition.12)))
...
ggplot(num_disposition, aes(x = Var1, Freq)) + geom_bar(stat = 'identity', fill="cornflowerblue") + theme_bw()+
  theme(axis.text.x = element_text(angle=90, vjust=0.6, size = 14)) + ggtitle("Call History by Disposition") + xlab("Disposition") + geom_text(aes(label = Freq), vjust = -0.6, size = 3) + theme(text = element_text(size=14))+ ylab("Frequency")
```
...
## Number of Incidents by shift Across Neighborhoods on `r MyDate`
\
...
## Number of Incidents Across Neighborhoods on `r MyDate`
\
...
## Number of Incidents by Hospitals on `r MyDate`
\
...
ggplot(hospital, aes(reorder(Var1,Freq), y = Freq)) + geom_bar(stat = 'identity', fill="lightskyblue") + theme_bw() + ggtitle("Number of incidents by hospitals") + xlab("Hospitals") +
  theme(axis.text.x = element_text(angle=90, vjust=0.6, size = 14)) +  geom_text(
    aes(label = Freq), 
    hjust = -0.5, size = 3,
    position = position_dodge(width = 1),
    inherit.aes = TRUE) + 
...
## Disposition Based on Neighborhoods on `r MyDate`
\
...
tmap_mode('view')
map1 <- tm_shape(nbhd) + tm_polygons('AssistUnit' ,style="cat", palette=pal, title = "Assist Unit") +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) 

tmap_mode('view')
map2 <- tm_shape(nbhd) + tm_polygons('Exposure' ,style="cat", palette=pal) +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) + tmap_options(limits = c(facets.view = 6))

tmap_mode('view')
map3 <- tm_shape(nbhd) + tm_polygons('Referred_Medical_Examiner', style="cat", palette=pal, title = "Referred Medical <br> Examiner") +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) 

tmap_mode('view')
map4 <- tm_shape(nbhd) + tm_polygons('PartyDeniesNeed', style="cat", palette=pal, title= "Party Denies Need") +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) 


tmap_mode('view')
map5 <- tm_shape(nbhd) + tm_polygons('Treated_Transported_by_EMS',style="pretty", palette=pal, title = "Treated Transported <br> by EMS") +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) 

tmap_mode('view')
map6 <- tm_shape(nbhd) + tm_polygons('PatientRefusal' ,style="cat", palette=pal, title="Patient Refusal") +tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) 
...
## Boston EMS Responses on `r MyDate`
\
...
## Boston EMS Responses Based on Neighborhoods on `r MyDate`
\
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=""}
...
tmap_mode('view')
tm_shape(nbhd2) + tm_polygons(c('ALS_','BLSA_','BLS_') , style="pretty", palette = list(pal, pal, pal), title = c("ALS Paramedic", "BLS AEMT", "BLS Basic / EMT"))+tm_basemap(server = c('OpenStreetMap'))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom")) +
